...
country_map_ws <- function(obj, entity) {
  # BrBG con amarillo al centro
  modified_palette <- c("#D8B365", "#FFEE99", "#5AB4AC")
  obj %>%
    mutate(
      Estado = factor(
        x = Estado,
        levels = c("activa o bueno", "regular", "inactiva o no reportada"))) %>% 
    ...
    geom_sf(aes(fill = Estado), shape = 21, size = 2.5, alpha = 0.8) +
    scale_fill_manual(
      values = c(
        # "activa o bueno" = viridis(3)[2],
        # "inactiva o no reportada" = viridis(3)[3],
        # "regular" = viridis(3)[1]
        "activa o bueno" = brewer.pal(6 , name = 'BrBG')[6],
        "regular" = brewer.pal(6 , name = 'BrBG')[3],
        "inactiva o no reportada" = brewer.pal(6 , name = 'BrBG')[1]
        # "activa o bueno" = modified_palette[3],
        # "regular" = modified_palette[2],
        # "inactiva o no reportada" = modified_palette[1]
      ),
      labels = c(
